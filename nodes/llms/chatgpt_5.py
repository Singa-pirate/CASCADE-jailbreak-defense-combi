...
class ChatGPT5(ProprietaryAPIBase):
    ...
    def generate(self, message_list, count_tokens=False):
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                logging.error(f"Warning: Rate limited by OpenAI API, retrying in {wait}s (attempt {attempt + 1}/{max_retries}): {e}")
                ...
                logging.error(f"Error: Bad request to OpenAI API: {e}")
                ...
                logging.error(f"Unexpected error when calling OpenAI API: {e}")
                if not is_final_attempt:
                    time.sleep(wait)
                continue
