...
class WildGuardOutputNode(DefenseNode):
    def __init__(self):
        super().__init__()
        self.label = Defense.WILDGUARD_OUTPUT
    ...
    def defend_and_update_state(self, messages: list[dict], prompt: str, response: str) -> dict:
        if self.predict_harmfulness(prompt, response):
            return {
                "rejected": True,
                "rejector": self.label,
    ...
    def predict_harmfulness(self, prompt: str, response: str) -> bool:
        formatted_prompt = WILDGUARD_INPUT_FORMAT.format(prompt=prompt, response=response)
